<script>
    ...
    const trackDetails = {
        1: { salary: '₹10L - 18L', role: 'CTI Specialist', tools: ['Maltego', 'Shodan', 'MISP'] },
        2: { salary: '₹8L - 14L', role: 'SOC Analyst', tools: ['Splunk', 'Wireshark', 'CrowdStrike'] },
        3: { salary: '₹12L - 24L', role: 'DevSecOps Engineer', tools: ['Docker', 'K8s', 'Snyk'] },
        4: { salary: '₹12L - 20L', role: 'GRC Analyst', tools: ['NIST', 'ISO 27001', 'OneTrust'] },
        5: { salary: '₹10L - 22L', role: 'Penetration Tester', tools: ['Burp Suite', 'Metasploit', 'Nmap'] },
        6: { salary: '₹14L - 28L', role: 'Security Engineer', tools: ['Python', 'Bash', 'Ansible'] }
    };
</script>
                ...
                <div>
                    ...
                    <div class="delivery-modes">
                        ...
                        <div class="mode-badge offline">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 21h18"/><path d="M5 21V7l8-4 8 4v14"/><path d="M17 21v-8.5a1.5 1.5 0 0 0-1.5-1.5h-7a1.5 1.5 0 0 0-1.5 1.5V21"/><path d="M9 21v-8.5"/></svg>
                            <span>In-Person Campus</span>
                        </div>
                    </div>
                </div>
                <MagneticButton href="/tracks/{activeTrackId}" variant="primary">Initialize Track</MagneticButton>
            </div>

            <div class="path-stats">
                <div class="stat-box">
                    <span class="label">Est. Compensation</span>
                    <div class="value pulse">{trackDetails[activeTrackId]?.salary || '₹12L+'}</div>
                </div>
                <div class="stat-box">
                    <span class="label">Target Role</span>
                    <div class="value">{trackDetails[activeTrackId]?.role || 'Specialist'}</div>
                </div>
                <div class="stat-box">
...
<style>
    ...
    .mode-badge.online svg { color: var(--accent-blue); }
    .mode-badge.offline svg { color: #10b981; }
    ...
    @media (max-width: 900px) {
        .path-header { flex-direction: column; gap: 2rem; align-items: stretch; }
