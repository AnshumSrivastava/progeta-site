...
{#if event}
    <div class="dashboard-container" style="padding-top:2rem;">
        <div class="section-header">
            ...
            <span class="sys-code">COMMUNITY EVENT <span style="color:var(--accent-primary)">//</span> {event.type.toUpperCase()}</span>
        </div>

        <div class="dashboard-panel" style="margin-bottom: 2rem;">
            <div class="panel-title"><span>EVENT OVERVIEW</span></div>
            <p style="font-size:1.3rem; line-height:1.6; color:var(--text-primary);">{event.description || 'No description available.'}</p>
        </div>
