...
<svelte:head>
    <title
        >Progeta Technologies | {workshop
            ? workshop.title
            : "Simulation Details"}</title
    >
</svelte:head>
...
{#if workshop}
    <div class="dashboard-container" style="padding-top:2rem;">
        <div class="section-header">
            ...
            <span class="sys-code"
                >LIVE FIRE SIMULATION <span style="color:var(--accent-primary)"
                    >//</span
                >
                {workshop.role
                    ? workshop.role.toUpperCase()
                    : "OPERATIVE"}</span
            >
        </div>
        ...
        <div
            ...
            style="margin-bottom:2rem; background: linear-gradient(135deg, rgba(255,255,255,0.03) 0%, rgba(20,20,20,0.5) 100%);"
        >
            <div class="panel-title">
                <span>MISSION DOSSIER</span> <span>[CLASSIFIED]</span>
            </div>
            <div
            ...
            >
                <div>
                    <div
                        style="font-family:var(--font-mono); color:var(--text-tertiary); font-size:0.8rem;"
                    >
                        OPERATIVE ROLE
                    </div>
                    <div
                        style="font-size:1.5rem; color:var(--accent-primary); margin-top:0.25rem;"
                    >
                        {workshop.role}
                    </div>
                </div>
                <div>
                    ...
                    <div
                        style="font-size:1.1rem; color:var(--text-primary); margin-top:0.25rem;"
                    >
                        {workshop.outcome}
                    </div>
                </div>
        ...
        <div class="module-dashboard">
            ...
            <div class="dashboard-panel">
                ...
                <div class="panel-title" style="margin-top:2rem;">
                    <span>SOFT SKILL REQUIREMENTS</span>
                </div>
                <div class="tool-grid">
                    {#if workshop.soft_mods && workshop.soft_mods.length > 0}
                        {#each workshop.soft_mods as m}
                            <a
                                ...
                                class="tool-chip"
                                style="border-color:var(--accent-secondary); color:var(--accent-secondary); text-decoration:none; cursor:pointer;"
                                >SP.{m.id} {m.title}</a
                            >
                        {/each}
                    {:else}
            ...
            <div class="dashboard-panel">
                <div class="panel-title">
                    <span>REQUIRED TECHNICAL LOADOUT</span>
                </div>
                <p
                    style="font-size:0.9rem; color:var(--text-secondary); margin-bottom:1rem;"
                >
                    Operatives must be proficient in the following technical
                    modules to succeed in this simulation.
                </p>
                <div
                    class="tool-grid"
