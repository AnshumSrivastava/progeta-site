...
{#if article}
    <div class="dashboard-container" style="padding-top:2rem;">
        <div class="section-header">
            ...
            <span class="sys-code">INTELLIGENCE BRIEFING <span style="color:var(--accent-primary)">//</span> {article.category}</span>
        </div>

        <div class="dashboard-panel" style="padding: 3rem;">
            <div style="border-left:4px solid var(--accent-primary); padding-left:1.5rem; margin-bottom:2rem;">
                 <div style="font-family:var(--font-mono); color:var(--text-tertiary); margin-bottom:0.5rem;">AUTHOR: {article.author} // DATE: {article.date}</div>
                 <div style="font-size:1.4rem; color:var(--text-secondary); font-style:italic;">"{article.summary}"</div>
            </div>
